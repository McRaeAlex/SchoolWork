...
int value(struct node *ref) { return ref->data; }
...
struct node *concat(struct node *ref1, struct node *ref2) {
    struct node *e = end(ref1);
    struct node *b = begin(ref2);
    e->next = b;
    b->prev = e;
 ...
 * \param ref1
 * \param ref2
 * \return distance is positive if ref1 appears before ref2 and negative if ref2
 * appears before ref1
 */
int distance(struct node *ref1, struct node *ref2) {
    ...
    int dist1 = 0;
    int dist2 = 0;
    for (struct node *i = head; i != NULL && i != ref1; i = i->next) {
        dist1++;
    }
    for (struct node *i = head; i != NULL && i != ref2; i = i->next) {
        dist2++;
    }
    return dist2 - dist1;
}
...
/*!
 * \brief unique Eliminates all but the first element from every consecutive
 * group of equivalent elements from the list \param ref A node form list
...
struct node *shift_left(struct node *ref, int n) {
    /*
    // Figure out the node we are to go after and delete node and then insert
    node in correct position
    // If we go all the way to the front just insert at front.
    struct node* left = ref;
    for (int i = 0; i < n; ++i) {
        left = left->prev;
        if (!left) {
            break;
    ...
    erase(ref);
    if (left->prev) {
        insert(left->prev, data);
    } else {
        push_front(left, data);
    }
    */
    struct node *temp = begin(ref); // Unsure if this is correct
    for (int i = 0; i < n; ++i) {
        if (temp->prev) {
            temp = erase(temp->prev);
        } else {
            temp = pop_front(temp);
        }
    }
 ...
 */
void minmax(struct node *ref, int *min, int *max) { /* Your code here */
}

/*!
 * \brief includes Returns true if the list starting from ref2 node is a subset
 * of the list starting from ref1. \param ref1 \param ref2 \return
 */
bool includes(struct node *ref1, struct node *ref2) {
    for (struct node *i = ref1; i != NULL; i = i->next) {
        if (i->data == ref2->data) {
            bool same = true;
            for (struct node *j = i, *k = ref2; j != NULL && k != NULL; j = j->next, k = k->next) {
                if (j->data != k->data) {
                    same = false;
                    break;
                }
