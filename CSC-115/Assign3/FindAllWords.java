...
public class FindAllWords {
    public static void main(String args[]) {
        ...
        try {
             ...
             */
            try {
                BufferedReader file = new BufferedReader(new FileReader(new File(args[1])));
                for (String line; (line = file.readLine()) != null;) {
                    String result = gb.isWord(line.toUpperCase());
                    if (result != null) {
                        System.out.println(String.format("%s: %s", line.toUpperCase(), result));
                    }
                }
                file.close();
            } catch (IOException e) {
