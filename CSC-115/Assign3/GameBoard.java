...
public class GameBoard {
    ...
    private static String[] GAME_PIECES = { "RIFOBX", "IFEHEY", "DENOWS", "UTOKND", "HMSRAO", "LUPETS", "ACITOA",
    ...
    public GameBoard(String[] start) {
        ...
        for (int row = 0; row < NUM_ROWS; row++) {
            for (int col = 0; col < NUM_COLS; col++) {
                this.board[row][col] = ((row < start.length && col < start[row].length())
    ...
    public String isWord(String word) {
        for (int i = 0; i < this.board.length; i++) { // loops through rows
            for (int j = 0; j < this.board[i].length; j++) { // loops through col
                if (this.board[i][j] == word.charAt(0)) { // check first character
                    Stack<String> lettersStack = new Stack<String>();
                    lettersStack.push(String.format("(%d,%d)", i, j));
                    this.visited[i][j] = true;
                    if (this.findPath(word, lettersStack, i, j)) { // if the word is found return it
                        this.resetVisited();
                        return this.formatResult(lettersStack);
                    }
                    this.resetVisited();
                }
            }
        }
        return null; // if youve gone through everything return null
    }
    ...
    private boolean findPath(String wordToFind, Stack<String> pathSoFar, int row, int col) {
        // if we have not gotten to the end of the word keep looking
        if (wordToFind.length() > pathSoFar.size()) {
            // what char are we looking at?
            char charToFind = wordToFind.charAt(pathSoFar.size());
            // if we dont go out of bound look down
            if (this.board.length > row + 1 && !this.visited[row + 1][col] && charToFind == this.board[row + 1][col]) {
                // add it to the stack
                pathSoFar.push(formatCoords(row + 1, col));
                // add it to visited
                this.visited[row + 1][col] = true;
                // call findPath
                // if its true return true
                if (this.findPath(wordToFind, pathSoFar, row + 1, col)) {
                    return true;
                }
                // if its false remove it from the stack and visited
                pathSoFar.pop();
                this.visited[row + 1][col] = false;
                // look at other options
            }
            // look up
            if (row - 1 >= 0 && !this.visited[row - 1][col] && charToFind == this.board[row - 1][col]) {
                // add it to the stack
                pathSoFar.push(formatCoords(row - 1, col));
                // add it to visited
                this.visited[row - 1][col] = true;
            ...
            // look right
            if (this.board[0].length > col + 1 && !this.visited[row][col + 1]
                    && charToFind == this.board[row][col + 1]) {
                // add it to the stack
                pathSoFar.push(formatCoords(row, col + 1));
                // add it to visited
                this.visited[row][col + 1] = true;
                // call findPath
                // if its true return true
                if (this.findPath(wordToFind, pathSoFar, row, col + 1)) {
                    return true;
                }
                // if its false remove it from the stack and visited
                pathSoFar.pop();
                this.visited[row][col + 1] = false;
                // look at other options
            }
            // look left
            if (col - 1 >= 0 && !this.visited[row][col - 1] && charToFind == this.board[row][col - 1]) {
                // add it to the stack
                pathSoFar.push(formatCoords(row, col - 1));
                // add it to visited
                this.visited[row][col - 1] = true;
            ...
            // look down and right
            if (this.board.length > row + 1 && this.board[0].length > col + 1 && !this.visited[row + 1][col + 1]
                    && charToFind == this.board[row + 1][col + 1]) {
                // add it to the stack
                pathSoFar.push(formatCoords(row + 1, col + 1));
                // add it to visited
                this.visited[row + 1][col + 1] = true;
                // call findPath
                // if its true return true
                if (this.findPath(wordToFind, pathSoFar, row + 1, col + 1)) {
                    return true;
                }
                // if its false remove it from the stack and visited
                pathSoFar.pop();
            ...
            // look down left
            if (this.board.length > row + 1 && col - 1 >= 0 && !this.visited[row+1][col-1]
                    && charToFind == this.board[row + 1][col - 1]) {
                // add it to the stack
                pathSoFar.push(formatCoords(row + 1, col - 1));
                // add it to visited
            ...
            // look up left
            if (row - 1 >= 0 && col - 1 >= 0 && !this.visited[row-1][col-1] && charToFind == this.board[row - 1][col - 1]) {
                // add it to the stack
                pathSoFar.push(formatCoords(row - 1, col - 1));
                // add it to visited
                this.visited[row - 1][col - 1] = true;
            ...
            // look up right
            if (row - 1 >= 0 && this.board[0].length > col + 1 && !this.visited[row - 1][col + 1]
                    && charToFind == this.board[row - 1][col + 1]) {
                // add it to the stack
                pathSoFar.push(formatCoords(row - 1, col + 1));
                // add it to visited
            ...
            // if you have looked every direction and cant find the next letter
            // it cant be found
            return false;
    ...
    private String formatResult(Stack<String> coords) { // done 
        String result = "]";
        while (!coords.isEmpty()) {
            result = String.format(", %s%s", coords.pop(), result);
        }
        result = "[" + result.substring(2);
        return result;
    }

    private String formatCoords(int row, int col) { // done
        return String.format("(%d,%d)", row, col);
    }

    private void resetVisited() { // done
    ...
    static public void main(String[] args) {
        ...
        GameBoard test02 = new GameBoard(3);
        //System.out.println(test02);
        System.out.println(test02.isWord("setito"));
    }
}
